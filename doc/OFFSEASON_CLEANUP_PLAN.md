...
| **BIOBUZZ vision: balls on the Limelight, Hive tags on a webcam** | researched and designed 2026-09-16, `doc/VISION_TWO_CAMERAS.md`: Hive-Vision SSD model checked (right format for the 3A, unmeasured), Limelight pitched ~30 deg down for balls only, a USB webcam with the SDK BIOBUZZ cluster library for Hive line-up and UP/DOWN, relocalization parked (every tag is on the seesaw Hive). `LimelightBalls` / `WebcamTags` adapters behind the existing `TagSighting` / `AimTarget` seams, bench checks V1-V9. Nothing built | |
...
| **teams/starterbot2027** (BioBuzz repo) | built 2026-09-16: P3 Starterbot and GG Starterbot, the goBILDA BIOBUZZ StarterBot (mecanum, intake Roller, Launcher with the windmill feeder) on goBILDA's controls and device names; no Pinpoint, Limelight or Pedro yet, README says the order to add them. Never run on a robot | |
